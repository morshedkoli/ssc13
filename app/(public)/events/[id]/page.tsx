...
interface Participant {
    ...
    contribution?: number;
    remarks?: string;
    member: { id: string; name: string; phoneRaw: string };
}
        ...
        <div className="page-container py-8 md:py-10 space-y-5">
            ...
            <section className="grid sm:grid-cols-2 gap-4">
                <div className="surface-card p-5">
                    ...
                    <p className="text-3xl font-extrabold text-[var(--text)]">{ev.participants.length}</p>
                </div>
                <div className="surface-card p-5">
                    <p className="text-sm text-[var(--text-muted)]">Total Contribution</p>
                    <p className="text-3xl font-extrabold text-[var(--text)]">BDT {totalContrib.toLocaleString()}</p>
                </div>
            </section>

            <section className="surface-card p-5 sm:p-6">
                <h2 className="text-xl font-bold text-[var(--text)] mb-4">Participants</h2>
                {ev.participants.length === 0 ? (
                    <EmptyState title="No participants yet" description="Participants will appear once members are added to this event." />
                ) : (
